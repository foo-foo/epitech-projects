...
// Last update Sun Apr 26 16:30:42 2015 Anatole Menichetti
//
...
Kitchen::Kitchen(Parser const &parameters, std::queue<Command *> &commands, size_t const name,
  ...
  if (!_pid)
    {
      ...
      cmd1.kitchen = 0;
      std::cout << "read cmd : " <<read(this->_receive, &cmd, sizeof(t_cmd))<<std::endl;
...
void    Kitchen::operator<<(Resources const &)
{
  int	ret;
...
void	Kitchen::operator<<(t_cmd const &cmd)
{
  int ret;
...
void    Kitchen::operator<<(KitchenStat const &nb)
{
  int ret;
