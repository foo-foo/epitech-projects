...
// Last update Sat Apr 25 14:57:19 2015 Sebastien Lauret
//

#include <algorithm>
#include "../includes/Order.hpp"
#include "../includes/RuntimeError.hpp"
#include "../includes/BadCommandError.hpp"
	     ...
	     std::map<std::string, PizzaType>& mPizzaType)
{
 ...
 // if (line.empty())
  ...
  while (!line.empty() && pos < line.size())
    {
...
size_t		Order::checkCommand(std::string command,
...
void	Order::_createCommand(std::string command,
			      std::map<std::string, PizzaSize>& mPizzaSize,
			      std::map<std::string, PizzaType>& mPizzaType)
{
  PizzaSize		pizza_size;
