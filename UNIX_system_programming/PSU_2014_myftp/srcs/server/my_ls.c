...
** Last update Sat Mar 28 15:07:48 2015 Anatole Menichetti
*/
...
char		*long_listing(int fd, char *n, struct stat *stat)
{
  ...
  t = 3;
  if (lstat(n, stat) == -1)
    return (my_rerror(ENOTTAKEN, 0, fd, 0));
...
char		*my_ls(int fd, FILE *fp, char *param, char **pwds)
{
  t_dir		**files;
  char		*ret;
  int		size;
  ...
  if (!(ret = get_scan_path(&files, fd, param, &n)) ||
      (strlen(ret) > 2 && ret[strlen(ret) - 2] == '\r'))
    return (ret);
  if (ret[0] != '\0' && (param[strlen(param)] = '\n'))
    return (my_nls(fd, fp, param, pwds));
  ret = strdup(FSTATOK);
  if (!ret)
